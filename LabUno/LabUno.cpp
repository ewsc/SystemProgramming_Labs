...
switch (uMsg) {
case WM_CREATE: {
hBitmap = (HBITMAP) LoadImage(nullptr, "RES/todd.bmp", IMAGE_BITMAP, 0, 0,
                              LR_LOADFROMFILE | LR_CREATEDIBSECTION | LR_DEFAULTSIZE);
if (hBitmap == nullptr) {
MessageBox(hwnd,
"Failed to load the image.", "Error", MB_OK | MB_ICONERROR);
return - 1;
}
return 0;
}
...
TransparentBlt(hdc, x, y, bm
.bmWidth, bm.bmHeight, hdcMem, 0, 0, bm.bmWidth, bm.bmHeight, transparentColor);
...
if (delta > 0) {
if (
IsPointInsideWindow(hwnd, xPos, yPos
- step * 2)) {
yPos -=
step;
} else {
